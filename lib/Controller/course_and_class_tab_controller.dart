...
class CourseClassQuizTabController extends GetxController
  ...
  final List<Tab> myTabs = <Tab>[
    ...
    Tab(
      child: Container(
        ...
        child: Text("${stctrl.lang["Courses"]}"),
      ),
    ),
    ...
    Tab(
      child: Container(
        padding: const EdgeInsets.only(left: 12.0, right: 12),
        child: Text("${stctrl.lang["Classes"]}"),
      ),
    ),
  ...
  var myCoursesSearch = <MyCourseModel>[].obs;

  var allMyClassSearch = <MyClassModel>[].obs;
  ...
  onCourseSearchTextChanged(String text) async {
    if (tabController?.index == 0) {
      ...
      myCoursesController.myCourses.forEach((value) {
        if (value.title != null) {
          if (value.title!.toUpperCase().contains(text.toUpperCase())) {
      ...
      myClassController.allMyClass.forEach((value) {
        if (value.title != null) {
          if (value.title!.toUpperCase().contains(
