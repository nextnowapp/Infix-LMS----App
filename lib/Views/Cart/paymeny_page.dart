...
import 'package:lms_flutter_app/Views/Cart/stripe/stripe_controller.dart';
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  Future createOrder() async {
    ...
    await http
      ...
      print('error razorpay::::: ' + err.toString());
    });
  }
                          ...
                          title: Row(
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child: OctoImage(
                                  image: NetworkImage(
                                      "${controller.paymentList[position].logo}"),
          ...
          child: ElevatedButton(
            onPressed: () async {
              if (controller.selectedGateway.value == "Wallet") {
                ...
                getAmount().then((value) async {
                  var result = '';

                  String token = await RemoteServices.getPayTMTxnToken(
                    userId: dashboardController.profileData.id.toString(),
                    amount: controller.paymentAmount.value,
                    currency: payTMCurrency,
                    orderId: controller.cartController.trackingIdPayTM.value,
                  );

                  try {
                    var response = AllInOneSdk.startTransaction(
                      payTmMerchantId,
                      controller.cartController.trackingIdPayTM.value,
                      controller.paymentAmount.value,
                      '$token',
                      '${callBackUrl}ORDER_ID=${controller.cartController.trackingIdPayTM.value}',
                      true,
                      false,
                    );
                    ...
                    response.then((value) {
                      print(value);
                      controller.makePayment('PayTM', value ?? Map());
                ...
                //   // amount: jsonEncode(controller.paymentAmount.value),
                //   amount: controller.paymentAmount.value,
